...
export async function encryptValue(
  ...
  const { Plaintext: plaintextKey, CiphertextBlob: encryptedKey } = await kmsClient.send(generateCmd);
 ...
 * 4. The plaintext data key is never stored.
 */
export async function decryptValue(kmsClient: KMSClient, userId: string, encrypted: EncryptedValue): Promise<string> {
  ...
  const decipher = crypto.createDecipheriv("aes-256-gcm", Buffer.from(plaintextKey), iv);
