...
describe("github_search_code", () => {
  test("1. allowlist accept — calls octokit.search.code with correct q param", async () => {
    const codeFn = vi.fn().mockResolvedValue({
      data: {
        ...
        items: [
          {
            path: "src/auth.ts",
            repository: { full_name: "kn-eng/kn-eng" },
            html_url: "https://github.com/kn-eng/kn-eng/blob/main/src/auth.ts",
          },
          {
            path: "src/middleware.ts",
            repository: { full_name: "kn-eng/kn-eng" },
            html_url: "https://github.com/kn-eng/kn-eng/blob/main/src/middleware.ts",
          },
        ],
      },
    });
    ...
    const result = await _executeSearch(
      { octokit, allowedRepos: ALLOWED_REPOS },
      { owner: "kn-eng", repo: "kn-eng", query: "auth middleware" },
    );
    ...
    expect(result).toMatchObject({
      ...
      items: [
        {
          path: "src/auth.ts",
          repository: "kn-eng/kn-eng",
          htmlUrl: "https://github.com/kn-eng/kn-eng/blob/main/src/auth.ts",
        },
        {
          path: "src/middleware.ts",
          repository: "kn-eng/kn-eng",
          htmlUrl: "https://github.com/kn-eng/kn-eng/blob/main/src/middleware.ts",
        },
      ],
    });
  });
  ...
  test("2. allowlist reject — returns error, octokit.search.code NOT called", async () => {
    ...
    const result = await _executeSearch(
      { octokit, allowedRepos: ALLOWED_REPOS },
      { owner: "evil-corp", repo: "secrets", query: "password" },
    );
  ...
  test("3. allowlist case-insensitive — KN-ENG/kn-eng is accepted", async () => {
    ...
    const result = await _executeSearch(
      { octokit, allowedRepos: ALLOWED_REPOS },
      { owner: "KN-ENG", repo: "kn-eng", query: "test" },
    );
  ...
  test('5. invalid query (status 422) → returns { error: "invalid_query" }', async () => {
    ...
    const result = await _executeSearch(
      { octokit, allowedRepos: ALLOWED_REPOS },
      { owner: "kn-eng", repo: "kn-eng", query: "x" },
    );
...
describe("github_get_file", () => {
  test("6. allowlist accept — calls octokit.repos.getContent, returns decoded content", async () => {
    ...
    const result = await _executeGetFile(
      { octokit, allowedRepos: ALLOWED_REPOS },
      { owner: "kn-eng", repo: "kn-eng", path: "src/hello.ts" },
    );
  ...
  test("7. allowlist reject (getFile) — returns error, no API call", async () => {
    ...
    const result = await _executeGetFile(
      { octokit, allowedRepos: ALLOWED_REPOS },
      { owner: "evil-corp", repo: "secrets", path: "passwords.txt" },
    );
  ...
  test("8. truncation — file > 50KB → truncated: true, content.length === 50*1024", async () => {
    ...
    const result = await _executeGetFile(
      { octokit, allowedRepos: ALLOWED_REPOS },
      { owner: "kn-eng", repo: "kn-eng", path: "big.txt" },
    );
  ...
  test("9. no truncation — small file → truncated: false, full content returned", async () => {
    ...
    const result = await _executeGetFile(
      { octokit, allowedRepos: ALLOWED_REPOS },
      { owner: "kn-eng", repo: "kn-eng", path: "small.txt" },
    );
  ...
  test('10. directory response → returns { error: "is_directory" }', async () => {
    ...
    const getContentFn = vi.fn().mockResolvedValue({
      data: [{ type: "file", name: "index.ts", path: "src/index.ts" }],
    });
    const octokit = makeOctokit({ repos: { getContent: getContentFn } });

    const result = await _executeGetFile(
      { octokit, allowedRepos: ALLOWED_REPOS },
      { owner: "kn-eng", repo: "kn-eng", path: "src" },
    );
  ...
  test('11. 404 → returns { error: "not_found" }', async () => {
    ...
    const result = await _executeGetFile(
      { octokit, allowedRepos: ALLOWED_REPOS },
      { owner: "kn-eng", repo: "kn-eng", path: "nonexistent.ts" },
    );
