...
import { _executeGetWorkflowRunLogs, _executeListWorkflowRuns } from "../../src/tools/github/workflows.js";
  ...
  ({
    actions: {
      listWorkflowRunsForRepo:
        overrides.listWorkflowRunsForRepo ?? vi.fn().mockResolvedValue({ data: { workflow_runs: [] } }),
      listJobsForWorkflowRun: overrides.listJobsForWorkflowRun ?? vi.fn().mockResolvedValue({ data: { jobs: [] } }),
    },
    checks: {
      listAnnotations: overrides.listAnnotations ?? vi.fn().mockResolvedValue({ data: [] }),
    },
...
describe("_executeListWorkflowRuns", () => {
  ...
  it("uses default repo when owner/repo are omitted", async () => {
    ...
    expect(listFn).toHaveBeenCalledWith(expect.objectContaining({ owner: "kn-eng", repo: "kn-eng", per_page: 5 }));
    expect("runs" in result).toBe(true);
  });
});
...
describe("_executeGetWorkflowRunLogs", () => {
  ...
  it("returns failed job with annotations", async () => {
    ...
    expect(listAnnotationsFn).toHaveBeenCalledWith(expect.objectContaining({ check_run_id: 9001 }));
  });
