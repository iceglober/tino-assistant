...
import { buildAuthMiddleware, type AuthVariables } from '../../src/server/middleware/auth.js';
...
describe('auth middleware — 401 JSON for unauthenticated /api/* (gap #2)', () => {
  ...
  it('passes /api/config through to the route when session exists', async () => {
    const app = buildApp(
      stubAuth({ user: { id: 'u1', email: 'a@example.com', name: 'A' } }),
    );
    const res = await app.request('/api/config');
  ...
  it('returns 403 JSON when session email is outside allowedDomain', async () => {
    const app = buildApp(
      stubAuth({ user: { id: 'u1', email: 'evil@other.com', name: 'A' } }),
      'example.com',
    );
    const res = await app.request('/api/config');
  ...
  it('admits session whose email matches allowedDomain', async () => {
    const app = buildApp(
      stubAuth({ user: { id: 'u1', email: 'ok@example.com', name: 'A' } }),
      'example.com',
    );
    const res = await app.request('/api/config');
