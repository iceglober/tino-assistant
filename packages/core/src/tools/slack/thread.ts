...
const inputSchema = z.object({
  ...
  threadTs: z
    .string()
    .min(1)
    .describe("Thread timestamp (the ts of the parent message — get this from slack_search_messages results)"),
...
type ThreadResult = { messages: ThreadMessage[]; count: number; hasMore: boolean } | { error: string; message: string };
...
export async function _executeSlackReadThread(
  ...
  try {
    ...
    if (slackError === "missing_scope") {
      return {
        error: "missing_scope",
        message: "SLACK_USER_TOKEN is missing channels:history or groups:history scope.",
      };
    }
