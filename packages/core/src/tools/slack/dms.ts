...
const listDmsInputSchema = z.object({
  limit: z.number().int().min(1).max(50).default(20).describe("Max conversations to return (1–50, default 20)"),
  sinceIso: z
    .string()
    .optional()
    .describe(
...
const readDmInputSchema = z.object({
  channel: z.string().min(1).describe("DM channel ID (from slack_list_dms or slack_search_messages results)"),
...
type ListDmsResult = { conversations: DmConversation[]; count: number } | { error: string; message: string };

type ReadDmResult = { messages: DmMessage[]; count: number; hasMore: boolean } | { error: string; message: string };
...
export async function _executeListDms(
  ...
  try {
    ...
    for (const ch of allChannels) {
      ...
      const isConnect =
        (ch.is_ext_shared as boolean) === true || ((ch.connected_team_ids as string[] | undefined)?.length ?? 0) > 0;
