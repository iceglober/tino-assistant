...
export async function stepBaa(config: Partial<DeployConfig>): Promise<Partial<DeployConfig>> {
  displayStep(3, 6, 'BAA Verification');
  ...
  if (awsIdentity) {
    ...
    displayWarning('Could not automatically verify AWS BAA status (AWS Artifact API does not expose BAA status programmatically).');
