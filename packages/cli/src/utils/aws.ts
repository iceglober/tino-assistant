...
export async function listBedrockModels(region: string): Promise<BedrockModel[]> {
  try {
    const output = execSync(`aws bedrock list-inference-profiles --output json --region "${region}"`, {
      encoding: "utf8",
      stdio: ["pipe", "pipe", "pipe"],
      timeout: 15_000,
    });
 ...
 */
export async function validateGitHubToken(token: string): Promise<{ ok: boolean; login?: string; error?: string }> {
  try {
 ...
 */
export async function validateGitHubRepo(token: string, repo: string): Promise<{ ok: boolean; error?: string }> {
  try {
