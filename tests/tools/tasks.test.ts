...
import { scheduleTaskTool, listTasksTool, cancelTaskTool } from '../../src/tools/tasks.js';
...
describe('scheduleTaskTool', () => {
  ...
  it('calls taskStore.create with userId, description, and epoch seconds', async () => {
    ...
    const [calledUserId, calledDesc, calledEpoch] = (store.create as ReturnType<typeof vi.fn>).mock.calls[0] as [string, string, number];
  ...
  it('returns an error object when scheduledAtIso is not a valid date', async () => {
    ...
    const result = await tool.execute(
      { description: 'do something', scheduledAtIso: 'not-a-date' },
      {} as never,
    );
