...
describe('createSqliteHistoryStore', () => {
  ...
  it('round-trip across instance close/reopen', () => {
    ...
    expect(store2.get('U1')).toEqual(msgs);
  });

  it('cap trimming honored', () => {
  ...
  it('trim invariant: orphan tool messages skipped at trim boundary', () => {
    const store = createSqliteHistoryStore({ dbPath: tempDbPath(), cap: 5 });
